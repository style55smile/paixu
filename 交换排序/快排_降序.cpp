...
void  kuaipai(int *a, int left, int right ) {
	...
	while (left < right)
	{
		//从数组左面开始搜索，大于等于基准数的跳过，小于基准数的下标为left，如果搜索到基准数坐标跳出
		//此时，left=（1、小于基准数的下标。2、基准数的坐标  ）
		while (a[left] >= a[m_pivot] && left != m_pivot) {
			left++;
		}
		//从数组右面开始搜索，小于等于基准数的跳过，大于基准数的下标为right，如果搜索到基准数坐标跳出
		//此时，right=（1、大于基准数的下标。2、left的坐标）
		while (a[right] <= a[m_pivot] && left <right ) {
			right--;
		}
		...
		//如果 left < right 即
		//1、left没到基准数
		//2、right 坐标没到 left 坐标
		//3、right找到 大于基准数 的坐标，left找到 小于基准数 的坐标，可以交换
		if (left < right) {
			m_swap(&a[left], &a[right]);
		}
	}
	//交换 left 和 基准数  #此时 left == right
	//此时 left=（1、小于基准数的下标。2、基准数的坐标）
	m_swap(&a[m_pivot], &a[left]);
